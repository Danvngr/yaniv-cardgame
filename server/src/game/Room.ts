...
const NEXT_ROUND_WAIT_MS = 10000; // Wait for all players to click continue

export class GameRoom {
  private room: Room;
  private turnTimer: ReturnType<typeof setTimeout> | null = null;
  private stickingTimer: ReturnType<typeof setTimeout> | null = null;
  private playAgainHostTimer: ReturnType<typeof setTimeout> | null = null;
  private nextRoundTimeout: ReturnType<typeof setTimeout> | null = null;
  private nextRoundReadyPlayers = new Set<string>();
  private nextRoundPrepared = false;
  ...
  tryRejoinPlayer(name: string, avatar: string, newSocketId: string): string | null {
    ...
    const hadConnectedHuman = this.room.players.some(p => !p.isAi && p.isConnected);
  ...
  removeDisconnectedWaitingPlayers(): void {
    if (this.room.status !== 'waiting') return;

    const before = this.room.players.length;
    this.room.players = this.room.players.filter(p => p.isAi || p.isConnected);

    if (!this.room.players.some(p => p.odId === this.room.hostId)) {
      const nextHost = this.room.players.find(p => !p.isAi);
      if (nextHost) {
        nextHost.isHost = true;
        this.room.hostId = nextHost.odId;
      }
    }
  ...
  private executeAiTurn(aiId: string): void {
    ...
    this.onAiMove?.(aiId, discardOrderedCardsToThrow, drawFrom);
  ...
  executeMove(playerId: string, cardIds: string[], drawFrom: 'deck' | 'pile' | 'pileFirst' | 'pileLast' | 'pileIndex' | 'pileCardId', pileIndex?: number, pileCardId?: string, fromTimeout?: boolean): { success: boolean; message?: string } {
    ...
    // Add to discard pile
    this.room.discardPile.push(...discardOrderedCardsToThrow);

    // Track for sticking
    const lastThrownRank = discardOrderedCardsToThrow[discardOrderedCardsToThrow.length - 1].rank;

    // Draw card
    let drawnCard: Card | undefined;
    const playerName = player.name;
    console.log(`[DRAW-DEBUG] ${playerName} drawing from: ${drawFrom}, pile length: ${this.room.discardPile.length}, threw ${discardOrderedCardsToThrow.length} cards`);
    ...
    if (isPileDraw) {
      ...
      const pileLengthBeforeThrow = this.room.discardPile.length - discardOrderedCardsToThrow.length;
    ...
    if (!player.isAi) {
      ...
      this.onPlayerMove?.(playerId, discardOrderedCardsToThrow, drawFrom, drawnCard);
    }

    // Update last discard group to the cards just thrown
    this.room.lastDiscardGroup = [...discardOrderedCardsToThrow];
  ...
  callYaniv(playerId: string): { success: boolean; message?: string } {
    ...
    if (remaining.length <= 1 || eliminated.length > 0) {
      ...
      this.scheduleNextRoundWait();
    }

    return { success: true };
  }

  markReadyForNextRound(playerId: string): { success: boolean; message?: string } {
    if (this.room.status !== 'roundEnd') {
      return { success: false, message: 'Round is not waiting for continue' };
    }

    const player = this.room.players.find(p => p.odId === playerId);
    if (!player || !player.isConnected || player.isAi) {
      return { success: false, message: 'Player not found' };
    }

    this.nextRoundReadyPlayers.add(playerId);

    if (!this.nextRoundPrepared) {
      this.prepareNextRoundCards();
    }

    this.onGameStateUpdated?.(this.toClientGameState(playerId), playerId);
  ...
  private scheduleNextRoundWait(): void {
    this.clearNextRoundWait();
    this.nextRoundReadyPlayers.clear();
    this.nextRoundPrepared = false;

    for (const p of this.room.players) {
      if (p.isAi) {
        this.nextRoundReadyPlayers.add(p.odId);
      }
    }
  ...
  private clearNextRoundWait(): void {
  ...
  private allPlayersReadyForNextRound(): boolean {
    const humans = this.room.players.filter(p => !p.isAi && p.isConnected);
    return humans.length > 0 && humans.every(p => this.nextRoundReadyPlayers.has(p.odId));
  }

  private prepareNextRoundCards(): void {
  ...
  private resetToWaiting(): void {
    ...
    for (const p of this.room.players) {
      ...
      if (!p.isAi) {
        p.isConnected = false;
        delete this.room.visitorIdToSocketId[p.odId];
      }
    }
    this.onRoomUpdated?.(this.toClientRoom());
  ...
  toClientRoom(): ClientRoom {
    const visiblePlayers = this.room.status === 'waiting'
      ? this.room.players.filter(p => p.isAi || p.isConnected)
      : this.room.players;
    ...
    return {
      ...
      players: visiblePlayers.map(p => this.toClientPlayer(p)),
    };
  }
  ...
  toClientGameState(forPlayerId?: string): ClientGameState {
    ...
    const waitingForRoundStart = this.room.status === 'roundEnd' && this.nextRoundPrepared;
    const isReadyPlayer = !!forPlayerId && this.nextRoundReadyPlayers.has(forPlayerId);
    ...
    return {
      ...
      currentTurnOdId: this.room.status === 'playing' ? (currentPlayer?.odId || '') : '',
      turnStartTime: this.room.turnStartTime,
      roundNumber: this.room.roundNumber,
      yourCards: player && (!waitingForRoundStart || isReadyPlayer) ? [...player.cards] : undefined,
