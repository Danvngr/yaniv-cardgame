...
const STORAGE_NOTIFICATIONS = 'settings:notifications';
const STORAGE_CHAT_ENABLED = 'settings:chatEnabled';
...
export default function SettingsScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [savedNotifications, savedChatEnabled] = await Promise.all([
          AsyncStorage.getItem(STORAGE_NOTIFICATIONS),
          AsyncStorage.getItem(STORAGE_CHAT_ENABLED),
        ]);

        if (!mounted) return;
        if (savedNotifications !== null) setNotifications(savedNotifications === 'true');
        if (savedChatEnabled !== null) setChatEnabled(savedChatEnabled === 'true');
      } catch {}
    })();
  ...
  const updateNotifications = (value: boolean) => {
    ...
    AsyncStorage.setItem(STORAGE_NOTIFICATIONS, String(value)).catch(() => {});
  };

  const updateChatEnabled = (value: boolean) => {
    setChatEnabled(value);
    AsyncStorage.setItem(STORAGE_CHAT_ENABLED, String(value)).catch(() => {});
  };
        ...
        <View style={styles.section}>
          ...
          <View style={styles.settingsCard}>
            ...
            <View style={[styles.settingRow, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
              ...
              <Switch
                value={chatEnabled}
                onValueChange={updateChatEnabled}
