...
export default function GameScreen() {
  ...
  const playAgainJoinAttemptedRef = useRef(false);
  ...
  const text = {
    en: {
      ...
      removePlayerMessage: (name?: string) => `Remove ${name || 'this player'} from the room?`,
      cancel: 'Cancel',
      remove: 'Remove',
      shareMessage: (code: string | number) => `Come play Yaniv with me! Room code: *${code}*`,
      missingRoom: 'No active room. Please create a new room',
      host: 'Host',
      inGame: 'In game',
      sent: 'Sent',
      inviteSentTitle: 'Invite Sent',
      inviteSentMessage: (name: string) => `Invite sent to ${name}`,
      inviteAlreadySentTitle: 'Invite Already Sent',
      inviteAlreadySentMessage: 'Invite already sent to this friend',
      error: 'Error',
      close: 'Close',
    ...
    he: {
      ...
      removePlayerMessage: (name?: string) => `האם להסיר את ${name || 'השחקן'} מהחדר?`,
      cancel: 'ביטול',
      remove: 'הסר',
      shareMessage: (code: string | number) => `בוא לשחק איתי יניב! קוד החדר: *${code}*`,
      missingRoom: 'אין חדר פעיל. צריך ליצור חדר חדש',
      host: 'מארח',
      inGame: 'במשחק',
      sent: 'נשלח',
      inviteSentTitle: 'הזמנה נשלחה',
      inviteSentMessage: (name: string) => `הזמנה נשלחה ל-${name}`,
      inviteAlreadySentTitle: 'הזמנה כבר נשלחה',
      inviteAlreadySentMessage: 'הזמנה כבר נשלחה לחבר זה',
      error: 'שגיאה',
      close: 'סגור',
      inviteFailed: 'לא הצלחנו לשלוח הזמנה',
    }
  };
  ...
  useEffect(() => {
    ...
    socketService.onReconnectFailed = (reason) => {
      ...
      setErrorMessage(t.reconnectFailed);
    ...
    if (isOnlineMode && params.roomCode && profile && !playAgainJoinAttemptedRef.current) {
      const myId = socketService.getMyPlayerId();
      const isInVisibleRoom = !!currentRoom?.players.some(p => p.odId === myId && p.isConnected);
      if (!isInVisibleRoom) {
        playAgainJoinAttemptedRef.current = true;
    ...
    if (!currentRoom && params.roomCode && !profile) {
  ...
  }, [isOnlineMode, room?.code, user, profile, params.roomCode]);
  ...
  const removePlayer = (playerId: string) => {
    ...
    Alert.alert(
      t.removePlayerTitle,
      t.removePlayerMessage(playerToRemove?.name),
      [
  ...
  const handleShare = async () => {
    try {
      await Share.share({
        message: t.shareMessage(String(roomCode)),
      });
    } catch (error) {
      console.log(error);
    }
        ...
        <View style={styles.playersSection}>
            {missingRoomWarning && (
              <View style={styles.missingRoomBanner}>
                <Text style={styles.missingRoomText}>{t.missingRoom}</Text>
              </View>
            )}
                ...
                if (slot.type === 'player') {
                  ...
                  return (
                    <View key={p.id} style={styles.playerCard}>
                      ...
                      {p.isHost && <Text style={styles.hostLabel}>{t.host}</Text>}
                      ...
                      <Pressable
                        ...
                        onPress={async () => {
                          ...
                          try {
                            ...
                            Alert.alert(
                              t.inviteSentTitle,
                              t.inviteSentMessage(friend.username)
                            );
                          } catch (error: any) {
                            if (error.message === 'Invite already sent') {
                              Alert.alert(
                                t.inviteAlreadySentTitle,
                                t.inviteAlreadySentMessage
                              );
                            } else {
                              Alert.alert(
                                t.error,
